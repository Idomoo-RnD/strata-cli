...
| [sketch.md](shoot/sketch.md) | `strata sketch` with a plan: the grey-box 3D animatic — a camera plan rendered to MP4 as a **reference for generating a clip** (`--ref-video`), never a layer, never the deliverable | camera plan · animatic · grey box · block the shot · 3D sketch · sketch the camera move · a reference video I make myself · the cut timing the model copies · storyboard the camera · a 3D preview of the shot |
| [pen-sketch.md](shoot/pen-sketch.md) | `strata sketch` with a pen spec: a rough.js ink still — a pose, a composition or a concept as a **reference for generating an image or a clip** (`--ref-image`), never a layer | pose reference · pen sketch · storyboard frame · sketch a pose · composition sketch · where the subject stands · hand-drawn reference · rough.js · a drawing the model should follow |
...
| [tracking.md](recipes/tracking.md) | `strata track` — put type on a surface in footage (a corner pin) or make an element follow a subject (position keyframes); the only way to track, and the footage prompted to fit it | track · tracking · corner pin · match move · text on a sign · on the billboard · on the phone screen · a callout that follows · a label that follows the car · pinned to · sticks to · the track slipped · no flat quad found · --min-area · --smooth |
