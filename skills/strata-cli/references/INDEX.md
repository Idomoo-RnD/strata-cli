...
| [scene-js.md](scene-js.md) | the scene as a program: `scene_v1.mjs`, `strata js init`, the builder API (every method and the JSON it emits), what it compiles away | write the scene in JavaScript · scene file · .mjs · relative time · after() · beats · a loop of layers · reveal() · drift() · push() · check() · the CLI runs my JS · do I need node |
| [scene-js-patterns.md](scene-js-patterns.md) | the recurring builds as scene-file code, and the compact JSON → builder translation table | lower third as a function · beat-cut montage in code · a sub-comp placed three times · matte in code · camera push in code · placeholders in code · translate my JSON scene to the builder · which method emits this key |
