...
This is the craft manifesto that used to open `SKILL.md`: story, shots, composition, 3D and
camera, motion, kinetic type, transitions, atmosphere, rhythm, polish, and the design rules.
It is written in the first person on purpose — it is a stance, not a procedure. The hub
(`SKILL.md`) routes here; every number and rule below is unchanged from the original text.
