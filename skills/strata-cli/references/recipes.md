...
- [1. Text](#1-text)
  - [Tracking-in (letters spread → converge)](#tracking-in-letters-spread--converge)
  - [Rich multi-style headline (per-span colour + word entrance)](#rich-multi-style-headline-per-span-colour--word-entrance)
  - [Typewriter with a caret that follows the text](#typewriter-with-a-caret-that-follows-the-text)
  - [Line-by-line stagger](#line-by-line-stagger)
  - [Per-word bounce-in](#per-word-bounce-in)
  - [Wavy / travelling lift](#wavy--travelling-lift)
  - [RGB-split glitch (VERIFIED with channel masks)](#rgb-split-glitch-verified-with-channel-masks)
  - [Per-letter 3D flip-in](#per-letter-3d-flip-in)
  - [Colour sweep across letters](#colour-sweep-across-letters)
  - [Marker highlight sweep](#marker-highlight-sweep)
  - [Seamless loop — first frame equals last frame, invisibly](#seamless-loop--first-frame-equals-last-frame-invisibly)
  - [Rule wipe — a divider drawing on between two regions](#rule-wipe--a-divider-drawing-on-between-two-regions)
  - [Textured text — gradient / stripes / gold shine (VERIFIED)](#textured-text--gradient--stripes--gold-shine-verified)
- [2. Transitions](#2-transitions)
  - [Scale-through (A blasts past, B scales in)](#scale-through-a-blasts-past-b-scales-in)
  - [Whip-pan (motion-blur swish)](#whip-pan-motion-blur-swish)
  - [Light-leak bloom over the cut](#light-leak-bloom-over-the-cut)
  - [Clock wipe (radial wedge)](#clock-wipe-radial-wedge)
  - [Iris (circle) reveal](#iris-circle-reveal)
  - [3D flip transition](#3d-flip-transition)
  - [Speed-line streak](#speed-line-streak)
  - [Luma dissolve (textured)](#luma-dissolve-textured)
- [3. Motion](#3-motion)
  - [Anticipation + overshoot](#anticipation--overshoot)
  - [Float / bob idle loop](#float--bob-idle-loop)
  - [3D camera push-in](#3d-camera-push-in)
...
- [4. Masks](#4-masks)
  ...
  - [Photo through text (alpha matte)](#photo-through-text-alpha-matte)
  - [Knockout text (hole in a panel)](#knockout-text-hole-in-a-panel)
  - [Barn-door reveal](#barn-door-reveal)
  - [Soft gradient wipe](#soft-gradient-wipe)
- [5. Special FX](#5-special-fx)
  - [Neon glow pulse + flicker](#neon-glow-pulse--flicker)
  - [Depth-of-field focus pull](#depth-of-field-focus-pull)
  - [Chromatic aberration (RGB split breathing)](#chromatic-aberration-rgb-split-breathing)
  - [Ambient light leaks](#ambient-light-leaks)
  - [Lens flare sweep](#lens-flare-sweep)
  - [Corner-pin perspective turn](#corner-pin-perspective-turn)
  - [Mirror reflection](#mirror-reflection)
  - [Frosted-glass panel](#frosted-glass-panel)
  - [Colour grade (teal–orange duotone)](#colour-grade-tealorange-duotone)
  - [Long flat shadow](#long-flat-shadow)
- [6. Extras](#6-extras)
  - [Rich text — many styles in one block (VERIFIED)](#rich-text--many-styles-in-one-block-verified)
  - [Easing comparison — one ease per row](#easing-comparison--one-ease-per-row)
- [7. Data viz](#7-data-viz)
  - [Count-up number — NATIVE, and safe for personalization (VERIFIED by render)](#count-up-number--native-and-safe-for-personalization-verified-by-render)
  - [Bar chart (mask wipe L→R)](#bar-chart-mask-wipe-lr)
  - [Progress ring (expanding wedge)](#progress-ring-expanding-wedge)
  - [Stat bar / percentage fill](#stat-bar--percentage-fill)
  - [Line-chart draw (path mask sweep)](#line-chart-draw-path-mask-sweep)
  - [Parallax depth (3D) — MEASURED](#parallax-depth-3d--measured)
- [8. Tracking — put text ON a surface, or make an element FOLLOW a subject](#8-tracking--put-text-on-a-surface-or-make-an-element-follow-a-subject)
...
> A hand-built native-shape ring has nothing to swap, so every viewer sees the same numbers. Keep the
> reveal animation (mask wipe / scale / opacity) — it still works on the replaced image.
> The other valid route is `strata chart bars|donut|line`: its value labels personalize through
> `render --data`, its heights/sweeps are baked — so per-viewer heights are one emitted scene
> per row. See [personalization.md](personalization.md).
...
If the effect is chosen first and the clip is also being generated, design the image **and**
