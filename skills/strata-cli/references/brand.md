...
one, every session re-invents the palette, type sizes, corner treatment and motion, and the
fifth video looks unrelated to the first.

The artefact is **`.brand/brand.md`** in the user's project: I write it from the assets they
give me, they approve it, and from then on **every scene is authored from it**.
...
- ❗ If `.brand/brand.md` already exists I **read it and follow it** — never a silent
  rebuild. I update it only when the user says the brand changed, saying what I changed.
...
|---|---|
| **Images** (logo, screenshots, style frames, product shots) | Read and describe it; extract the palette **with code**, not by eye (below) |
...
| **Docs** (brand book PDF, style guide, deck, markdown) | Read them. Lift exact hexes, font names, spacing rules, tone-of-voice, legal/usage rules |
| **Fonts** (`.ttf`/`.otf` files) | Record the **absolute file path** — a family name alone is useless (trap below) |
...
| **An edited video / raw footage** | Frames for palette + type; [video-editing.md](video-editing.md) if it needs trimming first |
...
carries **the easing curves, durations and stagger** — what makes two videos feel like one
brand, and what I would otherwise guess. Routes, best first:
...
| **`.aep`** (binary) | RIFX container; undocumented, values are packed binary | last resort. Ask for `.aepx` instead — one menu item away |
| **A render of the comp** | frames via ffmpeg | if all else fails: palette + type + timing by observation |

**What I pull out, into the token block:**
...
- **typography** — font family + size + tracking + leading per text layer, and the *ramp*
  ...
  (ask — see the fonts trap in step 2).
...
- **motion (the prize)** — keyframe **timing** and **ease** per property. Convert AE
  ...
  cubic-bezier); record entrance duration, sibling stagger and transition style. That
  becomes the `motion:` block.
...
one I record the *observed* behaviour and note it in Known Gaps.
...
a throwaway script ([generative-fx.md](generative-fx.md) has the pattern). Count quantised
colours across the logo/frames, drop near-white/near-black
unless they're structural, keep the top 5–8 by frequency:
...
separate a document that works from one that fails at compile time:
...
1. **Fonts are FILE PATHS.** `"font": "Inter"` does not compile. Record
   ...
   plus confirmation it covers the glyphs the brand needs (currency, accents, ₪/€, emoji). A missing glyph crashes the export.
...
**Add the section the source template leaves out: MOTION.** The template was written for
still frames, so it declares motion out of scope. For video it is the single biggest
...
I write `.brand/brand.md`, then summarise: **palette + type + the 3 laws + what's
missing**. Cheap to fix now, expensive after ten videos are built on it. I also save the
assets I was given next to it (`.brand/assets/`) so paths stay valid.
...
1. **Read it before the storyboard** — it decides the canvas, palette, type ramp,
   components and motion, so it comes before layout choice.
2. **Tokens beat invention.** Every colour, size, font, shadow and ease comes from the
   document. If a shot needs something the brand lacks, I propose an addition rather than
   quietly inventing one.
3. **Layout is still free** (unless the brand fixes it): the document governs *atoms and
   laws*, [layouts.md](layouts.md) still picks the frame — *atoms are sacred, composition
   is free*.
...
5. **Run the Pre-Render Self-Audit** from the document before I call a video done — the
   brand's own checklist, on top of the craft check in SKILL.md.
6. **Version the scene files** per cut (`promo_v1.json` → `promo_final.json`) so a series
   built on one brand stays legible.

---

# §C — The template

Copy this into `.brand/brand.md` and fill every section. Keep the section order, and the
token block as **one fenced yaml block** (never `---` frontmatter — markdown viewers render
it as raw text and wreck the header). Delete a section only if it genuinely does not apply,
...
description: >
  ...
  # State what was adapted for video (composition, sizing, motion) and
...
## Overview
<!-- 2-3 paragraphs + bullets. Name the aesthetic in one bolded phrase, enumerate the laws
every element must obey, then the typographic voice, the colour rhythm across shots and the
depth model. Close with "Key characteristics at frame scale:" - 5-7 bolded one-liners on
borders/shadows, palette behaviour, display type, corner rules, signature decorations,
density stance. -->
...
and ceilings), **Reference** (one vivid analog target + what failure looks like). Then
geometry: primary canvas + ratio, secondary ratios, safe area token, whether decorations may
bleed off-frame. -->
...
interchangeable, the CTA colour, single-purpose colours, the hard ceilings. -->

## Typography
<!-- Ramp architecture (reading/chrome vs display), then the hard rules: legibility floor,
how headline size follows word count / measure, casing + weight + tracking laws per ramp,
explicit "no ___" violations. Restate the font FILE paths. -->
...
## Shapes
<!-- Corner and geometry law in 1-2 bullets with named exceptions, plus the radius tokens
in px at the primary canvas (applied as `radius` on a mask `rect`). -->

## Components
<!-- Bullet digest of the token components grouped by role, one-line purpose each, plus
per-frame minimums. Each maps to a sub-comp in `comps`. -->

## Motion Language
<!-- The house motion, written out from the motion tokens above. State the tells to avoid
(everything fading in together, linear easing, dead frames). -->

<!-- HOW TO DERIVE THESE when the brand does not state them. Pick the archetype the brand's
own words imply, take its row as the starting point, write concrete values in.
...
**Silence** where the air lives. **Motion** its entrance/exit. **Fixed** non-negotiables.
**Free** what may vary. **Density** low/comfortable/dense. Cover at minimum: opener/cover,
card or catalog grid, a data/stat shot, a quote, a process/timeline, a closing plate. Name
the matching layout from layouts.md. -->
...
each scene references, plus what is MISSING and which treatment it blocks. -->
...
are guidance vs verified, treatments blocked by a missing asset, what the user still needs
to decide. -->
````
...
## Rules
- **`.brand/brand.md` is the source of truth** once it exists — read it, follow it, update
  it deliberately (never silently).
...
- **Keep the yaml token block machine-readable** — a later session (or another agent)
  parses it instead of the prose.
