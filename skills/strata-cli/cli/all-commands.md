...
| `sketch` | Make a reference sketch: a grey-box camera animatic (moves), or a rough.js pen drawing (a still); shot plan → MP4 · pen spec → PNG | local; ffmpeg for the animatic. Reference only, never a layer; later generation reference requires upload consent | `strata sketch plan.json -o plan.mp4` · [detail](../shoot/sketch.md) |
...
| `review` | Inspect timing, cuts, motion, legibility, narration vs bed and mix; MP4 → report and frame strips | local; ffmpeg. Writes evidence; does not decide aesthetic approval | `strata review scene_v1.mp4 --scene scene_v1.json -o review` · [detail](../qa/review.md) |
