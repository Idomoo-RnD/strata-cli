...
| `sketch` | Make a reference sketch: a grey-box camera animatic, or a rough.js pen drawing as a still or a short video; shot plan → MP4 · pen spec → PNG or MP4 | local; ffmpeg for video. Reference only, never a layer; later generation reference requires upload consent | `strata sketch plan.json -o plan.mp4` · [detail](../shoot/sketch.md) |
