...
- **Every text role is its own value.** A headline and its supporting paragraph are two placeholders even when they share a panel and a style; a badge or a label is content, not decoration. Shared style is never shared content, and unrelated media windows are never fed one slot to satisfy a slot count.
- **A flattened role is a blocking defect.** A headline present in the picture but baked into an image, or a paragraph dropped to fit a slot budget, fails delivery — it is not polish. Verify the real replacement keys against the compiled and tagged output (`--emit-timeline`, below), never against a descriptive label.
- **Prove the template in its own geometry.** Substitute realistic media and the longest and shortest copy into the SAME layout, type and motion, and review that render; a separately designed beauty render proves nothing about the template. Every delivered aspect ratio is reviewed on its own render.
