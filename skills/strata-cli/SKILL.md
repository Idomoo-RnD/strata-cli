---
name: strata-cli
description: Author cinematic motion-design videos with the `strata` CLI — compact scene JSON compiled to a binary `.idm` locally, assets (image, image-to-video, narration, music) generated via the Idomoo AI API, rendered to MP4. VASCO is a full 3D engine (camera, depth, masks, effects, per-character text animators). Use when the user asks to make a strata video, make/build/compile an IDM, build a motion-graphics / kinetic-text / title-sequence / explainer / promo / personalized or data-driven video, render an IDM to MP4, or animate layers with keyframes. ALSO use whenever generating an image — including with reference images (brand character, logo, product, style frame) passed as `--reference` and indexed in the prompt. ALSO for beat-synced/audio-reactive videos, textured/gradient text fills, confetti overlays, footage-in-shape masks, and product-occlusion ads (a shoe/bottle passing in front of the brand name). ALSO when turning a Figma design/frame into a video or scene. Not for the Idomoo cloud briefs/blueprints API.
---
