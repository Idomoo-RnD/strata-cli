...
([the protocol](ref-audio.md#--ref-audio--characters-that-speak-your-audio)).
...
- **Presenter, spokesperson, explainer host, testimonial → `generate video`** (portrait or
  ...
  the recipe and prompt in [video-generation-advanced.md](ref-audio.md#--ref-audio--characters-that-speak-your-audio)).
...
prompt + reference in **`.brand/brand.md`** ([brand.md](../brand/brand.md)) so later sessions reuse them.
...
- **Full-frame presenter (the usual case).** The image was generated *with* a background, so
  ...
  [blocks.md](../engine/blocks.md) and [video-layouts.md](../craft/video-layouts.md) §8.
