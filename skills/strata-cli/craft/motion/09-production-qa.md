...
- **Contrast**: text at least 4.5:1 against its background through the whole move, not only at rest - over footage that means a scrim matched to the plate's brightness ([blueprints.md](../../make/blueprints.md)), not a fixed opacity. **Minimum text size**: about 4% of frame height on mobile for body, 6%+ for headlines; larger for broadcast.
...
([personalization.md](../../engine/personalization.md)), and batch-render the edge rows with `render --data` ·
